...
class _OrdersHistoryScreenState extends State<OrdersHistoryScreen> {
  ...
  Future<void> _fetchOrders() async {
    ...
    try {
      ...
      if (_role.toLowerCase() == 'wholesaler') {
        list = await ProductService.fetchWholesalerOrders();
      } else {
        list = await ProductService.fetchBuyerOrders();
      }
      setState(() {
        _allOrders = list;
        _isLoading = false;
  ...
  Future<void> _updateStatus(int orderId, String newStatus) async {
    final success = await ProductService.updateOrderStatus(orderId, newStatus);
    if (success) {
  ...
  Future<void> _cancelOrder(int orderId) async {
    ...
    final success = await ProductService.updateOrderStatus(orderId, 'cancelled');
    if (success) {
      AppSnackbars.success(
