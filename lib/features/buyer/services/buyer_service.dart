...
class BuyerService {
  ...
  static Future<List<dynamic>> fetchCategories() async {
    try {
      final response = await http.get(Uri.parse('$baseUrl/categories'), headers: {'Content-Type': 'application/json'});
      final data = json.decode(response.body);
      if (response.statusCode == 200 && data['success'] == true) return data['data'] ?? [];
      return [];
    } catch (e) {
  ...
  static Future<List<dynamic>> fetchApprovedWholesalers() async {
    try {
      final response = await http.get(Uri.parse('$baseUrl/products/wholesalers'), headers: _headers);
      final data = json.decode(response.body);
      if (response.statusCode == 200 && data['success'] == true) return data['data'] ?? [];
      return [];
    } catch (e) {
  ...
  static Future<List<dynamic>> fetchBuyerOrders() async {
    try {
      final response = await http.get(Uri.parse('$baseUrl/orders/my-orders'), headers: _headers);
      final data = json.decode(response.body);
      if (response.statusCode == 200 && data['success'] == true) return data['data'] ?? [];
      return [];
    } catch (e) {
  ...
  static Future<List<dynamic>> fetchBuyerBids() async {
    try {
      final response = await http.get(Uri.parse('$baseUrl/negotiations/buyer'), headers: _headers);
      final data = json.decode(response.body);
      if (response.statusCode == 200 && data['success'] == true) return data['data'] ?? [];
      return [];
    } catch (e) {
  ...
  static Future<Map<String, dynamic>> fetchPublicSettings() async {
    try {
      final response = await http.get(Uri.parse('$baseUrl/settings/public'), headers: {'Content-Type': 'application/json'});
      final data = json.decode(response.body);
      if (response.statusCode == 200 && data['success'] == true) {
