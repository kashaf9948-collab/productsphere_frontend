...
class WholesalerService {
  ...
  static Future<List<dynamic>> fetchWholesalerProducts(int wholesalerId) async {
    try {
      final response = await http.get(Uri.parse('$baseUrl/products/wholesaler/$wholesalerId'), headers: _headers);
  ...
  static Future<List<dynamic>> fetchWholesalerOrders() async {
    try {
      final response = await http.get(Uri.parse('$baseUrl/orders/wholesaler'), headers: _headers);
      final data = json.decode(response.body);
      if (response.statusCode == 200 && data['success'] == true) return data['data'] ?? [];
      return [];
    } catch (e) {
  ...
  static Future<List<dynamic>> fetchWholesalerBids() async {
    try {
      final response = await http.get(Uri.parse('$baseUrl/negotiations/wholesaler'), headers: _headers);
      final data = json.decode(response.body);
      if (response.statusCode == 200 && data['success'] == true) return data['data'] ?? [];
      return [];
    } catch (e) {
