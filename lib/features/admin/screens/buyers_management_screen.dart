...
class _BuyersManagementScreenState extends State<BuyersManagementScreen> {
  ...
  Future<void> _fetchBuyers() async {
    ...
    final list = await AdminService.fetchAdminBuyers();
    setState(() {
      _buyers = list;
      _isLoading = false;
    });
  ...
  Future<void> _toggleAccess(dynamic buyer) async {
    ...
    final String newStatus = currentStatus == 'approved'
        ? 'suspended'
        : 'approved';
    ...
    final confirm = await showConfirmDialog(
      ...
      confirmColor: newStatus == 'suspended'
          ? AppTheme.expired
          : AppTheme.active,
    );
    ...
    if (result['success']) {
      AppSnackbars.success(
        ...
        message:
            "Marketplace access for '$name' has been ${newStatus == 'suspended' ? 'revoked' : 'restored'}.",
      );
      _fetchBuyers();
    } else {
      AppSnackbars.error(
